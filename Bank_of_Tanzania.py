...
class BOT():
    ...
    def get_Tressury_bill(self,Auc_number):
        '''
        Returns the Auction Results of a particular 
        '''
        url='http://www.bot.go.tz/FinancialMarkets/TBills/TBillsAuctionSummary.asp'
    ...
    def get_Tbils(self,number_of_results):
        ...
        if number_of_results is 0 :
            ...
            print(auctions)
        result=[[self.get_Tressury_bill(auc)] for auc in auctions]
        return result

    def stop(self):
        raise StopIteration
    
    def T_bill_profit(self,number_of_results):
        auction_values=self.get_Tbils(number_of_results)
        profit=[]
        for list_values in auction_values:
            for tuples_list in list_values:
                for tuples in tuples_list:
                    for value in tuples:
                        if tuples.index(value) != 0:
                            if tuples.index(value) is 1:
                                price=(100-float(value))/100
                                print(1+price)
                        else:
                            print(value)
        return profit
